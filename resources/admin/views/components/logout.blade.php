<template x-data="{}" x-if="$store.logOut.active">
    <div class="dialog confirmDialogLayout" x-data="selectOption"
        ...
        <div class="dialog-container modal-background hidden">
            <div class="confirmDialog modal" id="select-option" style="padding: 25px 35px;">
                <i class='bx bx-help-circle' style="margin-bottom: 12px;"></i>
                <h3 style="margin-bottom: 10px;">Sing Out</h3>
                <div class="textDelete">Are you want to Sing Out ?</div>
                <div class="btnActionConfirmDialog">
                <a href="{!! route('admin-sign-out') !!}" style="display: flex;justify-content: center;
    margin: 0 20px;"><button class="btnConfirm">
                    <span>Yes</span>
                </button></a>
                <button class="btnCancel" @click="close(false)" style="background: none;color: red;">Cancel</button>
                </div>
            </div>
        </div>
        <script>
