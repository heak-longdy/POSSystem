...
<template x-data="{}" x-if="$store.components.active">
    <div class="dialog" x-data="NotificationOption" x-bind:style="{ zIndex: $store.libs.getLastIndex() + 1 }"
    ...
    font-size: 18px;"></i>
        ...
        <script>
            Alpine.data('NotificationOption', () => ({
                data: null,
                loading: true,
                options: null,
                selected: [],
...
<script>
    ...
    window.NotificationOption = (options) => {
        Alpine.store('components', {
            active: true,
            options: {
