    ...
    <div class="content-wrapper form-admin" id="app" x-data="xIndex">
        ...
        <div class="ourServiceLayout">
            <div class="accordionGp">
                <h3 class="fontWeight"><i class='bx bxs-wrench'></i>Application Form</h3>
                <template x-for="(item,index) in data?.Interns">
                            ...
                            <div class="timelineLayout">
                                <div class="timeline">
                                    <div class="timeline-box" id="primary">
                                        <div class="icon">
                                            <img x-bind:src="baseImageUrl + item?.image" alt="" onerror="this.onerror=null;this.src='{{ asset('admin-public/logo/profile.png') }}';">
                                        </div>
                                        <div class="timeline-body">
                                            <template x-for="(valItem,valIndex) in item?.model_setting_de">
                                                <div class="timeLineItem" x-show="valItem?.title || valItem?.description">
                                                    <h4 class="header" x-text="valItem?.title ?? '---'"></h4>
            ...
            <div class="accordionGp">
                <h3 class="fontWeight"><i class='bx bx-eraser'></i>For Providers</h3>
                <template x-for="(item,index) in data?.Providers">
                    <div class="accordionItem" x-data="{ indexPanel: (item.type + index) }">
                        <div class="accordion" @click="active === indexPanel ? active = null : active = indexPanel">
                            <span><span class="lineWord"><span x-text="index+1"></span>.&nbsp;<span
                                        x-text="item.title"></span></span></span>
            ...
            <div class="accordionGp">
                <h3 class="fontWeight"><i class='bx bxs-widget'></i>Our vision goes beyond simply placing interns.
                    We want to foster a win-win development</h3>
                <template x-for="(item,index) in data?.Vision">
                    <div class="accordionItem" x-data="{ indexPanel: (item.type + index) }">
                        <div class="accordion" @click="active === indexPanel ? active = null : active = indexPanel">
                            <span><span class="lineWord"><span x-text="index+1"></span>.&nbsp;<span
                                        x-text="item.title"></span></span></span>
            ...
            {{-- <div class="actOurserFooter">
                <button type="button" class="btnSave"><i class='bx bx-save'></i>Save Change</button>
            </div> --}}
        </div>
    </div>
    @include('admin::pages.our_service.store')
@stop
    ...
    <script lang="ts">
        document.addEventListener('alpine:init', () => {
            Alpine.data('xIndex', () => ({
                ...
                dialogStore(item) {
                    this.$store.dialogOurService.open({
                        data: {
                            'digPosition': 'top',
                            ...item
                        },
                        afterClosed: (result) => {
                            if (result) {
                                ...
                                let service = this.data[dataRes?.type]?.find(service => service.id === dataRes?.id);
                                if (service) {
                                    service.title = dataRes?.title;
                                    service.status = dataRes?.status;
