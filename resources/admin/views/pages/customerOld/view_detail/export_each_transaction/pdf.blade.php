<div class="form-body-pdf">
    ...
    <table class="table" id="tableCustomerViewDetailIncome">
        <thead class="header_table">
            ...
            <tr>
                ...
                <th class="pdfBGColorFontText">លេខរៀងចំណូល</th>
            ...
            <tr>
                ...
                <th class="pdfBGColorFontText">IDRevenue</th>
        ...
        <tbody>
            @foreach ($data as $index => $item)
                <tr>
                    ...
                    <td class="pdfBorder">
                        CASH{{ $item->revenue_id }}</td>
                    <td class="pdfBorder">
                        {{ $item->revenue->user->name ?? '' }}</td>
                    <td class="pdfBorder">
                        <?php
                        $stringCurreny = [];
...
<script>
    $(document).ready(function() {
        const report = document.getElementById('tableCustomerViewDetailIncome');
        const opt = {
            fontSize: 10,
            margin: 0.1,
        ...
        html2pdf().set(opt).from(report).save().then((res) => {
            window.location.href = '{{ route('admin-customer-income', request('id')) }}';
        });
    });
</script>
