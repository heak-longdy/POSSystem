    ...
    <div class="form-admin" x-data="xStockOut">
        ...
        <form id="form" class="form-wrapper">
            <div class="form-header">
                <h3> <i data-feather="arrow-left" s-click-link="{!! route('admin-stock-out-list') !!}"></i>
            ...
            <div class="form-body">
                <template x-if="inputMutiple?.length > 0">
                    <template x-for="(item,index) in inputMutiple" :key="item.id">
                        <div class="inventoryLayoutCus stockOutLayout">
                            ...
                            <div class="itemInv">
                                <div class="form-row">
                                    <label>Sell To<span>*</span></label>
                                    <select name="sell_to" x-model="item.sell_to.value">
                                        @foreach ($stockTypes as $item)
                                            <option value="{{ $item->key }}">{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                    <template x-if="item.sell_to.error">
                                        <label class="error" x-text="'Sell to is required'"></label>
                                    </template>
                                </div>
                            </div>
                ...
                <div class="form-button justContentStart from-button-cus">
                    ...
                    <button color="danger" :disabled="loadingSubmit" type="button" s-click-link="{!! route('admin-stock-out-list') !!}">
                        <i data-feather="x"></i>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data("xStockOut", () => ({
                    ...
                    this.checkValidation((res) => {
                        if (res.length > 0) {
                            ...
                            Swal.fire({
                                ...
                                icon: "warning",
                                html: `Are you sure to ${this.btnSubmit} stock out and you can not recover back. ?`,
                ...
                checkValidation(callback) {
                    console.log(this.inputMutiple, 'fsfsfssfsfsfsffs');
                    let error = [];
                    if (this.inputMutiple.length > 0) {
                        this.inputMutiple.forEach(val => {
                            ...
                            if (!val.sell_to.value) {
                                val.sell_to.error = true;
                                error.push(val.sell_to.value);
                            } else {
                                val.sell_to.error = false;
                            }
                            if (!val.qty.value) {
                                val.qty.error = true;
