    ...
    @if ($barbers->count() > 0)
        <div class="table-wrapper">
            ...
            <div class="table-body">
                @foreach ($barbers as $index => $item)
                    <div class="column">
                        <div class="row table-row-5">
                            <span>{!! $barbers->currentPage() * $barbers->perPage() - $barbers->perPage() + ($index + 1) !!}</span>
                        </div>
                        <div class="row table-row-10">
            ...
            <div class="table-footer">
                @include('admin::components.pagination', ['paginate' => $barbers])
            </div>
        </div>
    @else
