        ...
        <div class="VerifyDoc">
            ...
            <div class="VerifyDocBody">
                <div class="fontWeight">Out of&nbsp;<span class="colorGreen" x-text="DataDocuments?.length"></span>
                    ...
                    been completed, and&nbsp;<span class="colorRed"
                    ...
                    <template x-for="item in DataDocuments">
                        <div class="VerifyItem">
                            ...
                            <div class="VerRight">
                                ...
                                <div class="VerBtnGp">
                                    <input type="checkbox" :id="item?.id" class="VerifyInputCheckBox" :checked="item.check" />
                                    <label :for="item?.id" class="itemCheck VerBtn btnCheck">
                                        <div class="VerifyInputCheckBoxIcon">
                                            <svg viewBox="0 0 24 24" class="uv-checkmark">
                                                <path d="M4.1,12.7 9,17.6 20.3,6.3" fill="none"></path>
                                            </svg>
                                        </div>
                                        <template x-if="!item?.check">
                                            <div class="btnCheckText" @click="itemCheck(item)">
                                        ...
                                        <template x-if="item?.check">
                                            <div class="btnCheckText" @click="undoCheck(item)">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                                    viewBox="0 0 24 24"
                                                    style="fill: rgba(0, 0, 0, 1);transform: ;msFilter:;">
                                                    <path
            ...
            <div class="VerifyDocFooter">
                <button type="button" class="fontWeight"><svg xmlns="http://www.w3.org/2000/svg" width="24"
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data("XVerifyDoc", () => ({
                ...
                init() {
                    ...
                    console.log(this.DataDocuments, "0000");
                },
                checkFileComplete() {
                    if (this.DataDocuments) {
                        this.DataDocuments.forEach(item => {
                            ...
                            if(this.DocumentCompletes?.length){
                                findItem = this.DocumentCompletes.find(val => val.id == item.id);
                            }
                            item.check = findItem ? true : false
                        });
                    }
                    console.log(this.DataDocuments,'checkFileComplete');
                },
                itemCheck(item) {
                    console.log(item, 'itemCheck');
                    this.DocumentCompletes.push(item);
                    console.log(this.DocumentCompletes, "itemCheck DocumentCompletes");
                    this.checkFileComplete();
                },
                undoCheck(item){
                    console.log(item, 'undoCheck');
                    this.DocumentCompletes = this.DocumentCompletes.filter(val => val.id != item?.id);
                    console.log(this.DataDocuments,'undoCheck DataDocuments');
                    this.checkFileComplete();
                }
            }));
        });
