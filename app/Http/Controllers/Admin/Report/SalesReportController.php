...
class SalesReportController extends Controller
{
    ...
    {
        $this->middleware('permission:report-sales-view|booking-view', [
    ...
    public function details(Request $req, $period)
    {
        ...
        if ($isDailyPeriod) {
            ...
            if (app()->getLocale() === 'km') {
                ...
                $periodLabel = 'ថ្ងៃ' . __('sales_report.days.' . $dayKey) . ' ទី' . $parsedDate->format('d') . ' ' . __('sales_report.months.' . $parsedDate->month) . ' ឆ្នាំ' . $parsedDate->format('Y');
            } else {
            ...
            if (app()->getLocale() === 'km') {
                $periodLabel = __('sales_report.months.' . $parsedMonth->month) . ' ឆ្នាំ' . $parsedMonth->format('Y');
            } else {
    ...
    /**
     * Aggregate daily sales grouped by DATE(booking_date)
     */
    private function aggregateDailySales($query)
    {
        $bookings = $query->with([
    ...
    /**
     * Aggregate monthly sales grouped by DATE_FORMAT(booking_date, '%Y-%m')
     */
    private function aggregateMonthlySales($query)
    {
        $bookings = $query->with([
            'shop',
            'barber',
